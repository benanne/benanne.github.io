...
The **≋ Deep Sea ≋** team consisted of [Aäron van den Oord](http://reslab.elis.ugent.be/aaron), [Ira Korshunova](https://www.linkedin.com/in/IraKorshunova), Jeroen Burms, Jonas Degrave, [Lionel Pigou](http://lpigou.github.io/), [Pieter Buteneers](https://twitter.com/pieterbuteneers) and myself. We are all master students, PhD students and post-docs at Ghent University. We decided to participate together because we are all very interested in deep learning, and a collaborative effort to solve a practical problem is a great way to learn.
...
Rescaling the images was necessary because they vary in size a lot: the smallest ones are less than 40 by 40 pixels, whereas the largest ones are up to 400 by 400 pixels. We experimented with various (combinations of) rescaling strategies. For most network, we simply rescaled the largest side of each image to a fixed length.
...
We augmented the data to artificially increase the size of the dataset. We used various affine transforms, and gradually increased the intensity of the augmentation as our models started to overfit more. We ended up with some pretty extreme augmentations parameters:
...
For fairly deep networks (10+ layers), we found that varying this parameter between 0 and 1/2 did not really affect the predictive performance. However, larger values for a significantly reduced the level of overfitting. This in turn allowed us to scale up our models further. We eventually settled on a = 1/3.
