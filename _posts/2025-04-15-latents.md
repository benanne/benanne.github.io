...
Below is a diagram illustrating this two-stage recipe. Networks whose parameters are learnt in the respective stages are indicated with a $$\nabla$$ symbol, because this is almost always done using gradient-based learning.
...
* the **adversarial loss**, which involves a **discriminator** network which is co-trained with the autoencoder, as in generative adversarial networks (GANs)[^gans]. The discriminator is trained to tell apart real input signals from reconstructions, and the autoencoder is trained to fool the discriminator into making mistakes. The goal is to improve the realism of the output, even if it means deviating further from the input signal.
...
It goes without saying that this generic recipe is often deviated from in one or more ways, especially for audio and video, but I have tried to summarise the most common ingredients found in almost all modern practical applications of this modelling approach.
...
(While discrete representations played an essential role in making latent autoregression work at scale, I wanted to point out that autoregression in continuous space has also been made to work well recently[^givt].)
...
Some early works did consider an end-to-end approach, jointly learning the latent representation and the diffusion prior[^scorelatent] [^diffusionpriors], but this didn't really catch on. Although avoiding multiple sequential stages of training is desirable from a practical perspective, the perceptual and computational benefits make it worth the hassle.
...
<!-- Removed this sentence because we've already explained it in the previous section: It is important to mention at this point that the two-stage approach is not the only way to tweak generative models to focus on perceptually relevant signal content, and we'll discuss this in more detail later. -->
...
From a purely mathematical perspective, this is surprising: if the latents are real-valued, the size of the grid and the number of channels shouldn't matter, because **the information capacity of a single number is already infinite** (neatly demonstrated by [Tupper's self-referential formula](https://en.wikipedia.org/wiki/Tupper%27s_self-referential_formula)). But of course, there are two practical limitations that restrict the amount of information a single component of the latent representation is able to carry:
...
Hansen-Estruch et al.[^vitok] recently shared [an extensive exploration](https://arxiv.org/abs/2501.09755) of latent space capacity and the various factors that influence it (their key findings are clearly highlighted within the text). There is currently a trend toward increasing the downsampling factor, and maintaining the TSR by also increasing the number of channels accordingly, in order to facilitate image and video generation at higher resolutions (see e.g. DCAE[^dcae], LTX-Video[^ltxvideo] and GAIA-2[^gaia2]).
...
* The aforementioned DCAE[^dcae] does not deviate too far from the original VQGAN recipe, only replacing the L2 regression loss (MSE) with L1 (MAE). It keeps the LPIPS perceptual loss and the PatchGAN[^patchgan] discriminator.
...
* Curation influences modelability, because **some kinds of information are much easier to model than others**. If the latents encode information about unpredictable noise in the input signal, that will make them less predictable as well.
...
* Use **generative priors**: co-train a (lightweight) generative prior with the autoencoder, and make the latents easy to model by backpropagating the generative loss into the encoder, as in LARP[^larp] or CRT[^crt]. This requires care, because it can lead to pathological behaviour: latents are easiest to model when they encode no information at all!

* Supervise with **pre-trained representations**: encourage the latents to be predictive of existing high-quality representations (e.g. DINOv2[^dinov2] features), as in VA-VAE[^vavae] or MAETok[^maetok].

* Encourage **equivariance**: make it so that certain transformations of the input (e.g. rescaling, rotations) produce corresponding latent representations that are transformed similarly, as in [AuraEquiVAE](https://huggingface.co/fal/AuraEquiVAE) and EQ-VAE[^eqvae]. The figure from the EQ-VAE paper that I used in [section 4](#tradeoff) shows the profound impact that this constraint can have on the spatial smoothness of the latent space. Skorokhodov et al.[^diffusability] came to the same conclusion based on [spectral analysis](https://sander.ai/2024/09/02/spectral-autoregression.html) of latent spaces: equivariance regularisation makes the latent spectrum more similar to that of the pixel space inputs, which improves modelability.
...
A specific class of autoencoders for learning latent representations deserves a closer look: those with **diffusion decoders**. While a more typical decoder architecture features a feedforward network that directly outputs pixel values in one pass, and is trained adversarially, an alternative that's gaining popularity is to use diffusion for the task of latent decoding (as well as for modelling the distribution of the latents). This impacts reconstruction quality, but it also affects what kind of representation is learnt.
...
Tokenisers used for language models tend to be lossless (e.g. BPE[^bpe], SentencePiece[^sentencepiece]), so the resulting tokens aren't usually viewed as "latents". The relative lack of redundancy in language has not stopped people from trying to learn lossy higher-level representations, though! The techniques used for perceptual signals may not carry over, but several other methods to learn representations at the sentence or paragraph level have been explored[^timecontrol] [^planner] [^lcm].
...
Thank you for reading what ended up being my longest blog post yet! To wrap up, here are some key takeaways:
...
* **The V in VAE is vestigial**: the autoencoders used in two-stage latent generative models are really KL-regularised AEs, and barely regularised at that.
* The combination of a **regression loss**, a **perceptual loss** and an **adversarial loss** is surprisingly persistent. Almost all practical implementations of autoencoders for latent representation learning are variations on this theme.
...
Several years ago, [I wrote on this blog](https://sander.ai/2020/09/01/typicality.html#right-level) that representation learning has an important role to play in generative modelling. Though it wasn't exactly prophetic at the time (VQ-VAE had already existed for three years), the focus in the representation learning community back then was firmly on high-level semantic representations that are useful for discriminative tasks. It was a lot more common to hear about generative modelling in service of representation learning, than vice versa. The arrival of VQGAN[^vqgan] a few months later was probably what cemented the path we now find ourselves on, and made two-stage generative modelling go mainstream.

<!-- 
TODO: need more figures throughout to break things up (especially section 6 I think?)
TODO: maybe illustrate 3D tensors (multiple channels) in section 5 to show what TSR is, what spatial and temporal compression are
TODO: can we illustrate MSE-only vs adv+LPIPS reconstruction? Maybe Figure 12 from the VQGAN paper? IN section 6.2
--> 
...
Thanks to my colleagues at Google DeepMind for various discussions, which continue to shape my thoughts on this topic! In particular, thanks to TODO. Thanks also to Ethan Smith, Simo Ryu, Theodoros Kouzelis, Rami Seid, Stefan Baumann and Jack Gallagher.



<!-- TODO: who else? Who from GDM? -->
...
[^rotationtrick]: Fifty, Junkins, Duan, Iyengar, Liu, Amid, Thrun, Ré, "[Restructuring Vector Quantization with the Rotation Trick](https://arxiv.org/abs/2410.06424)", International Conference on Representation Learning, 2025.

[^fsq]: Mentzer, Minnen, Agustsson, Tschannen, "[Finite Scalar Quantization: VQ-VAE Made Simple](https://arxiv.org/abs/2309.15505)", International Conference on Representation Learning, 2024.

[^lfq]: Yu, Lezama, Gundavarapu, Versari, Sohn, Minnen, Cheng, Birodkar, Gupta, Gu, Hauptmann, Gong, Yang, Essa, Ross, Jiang, "[Language Model Beats Diffusion -- Tokenizer is Key to Visual Generation](https://arxiv.org/abs/2310.05737)", International Conference on Representation Learning, 2024.
...
[^larp]: Wang, Suri, Ren, Chen, Shrivastava, "[LARP: Tokenizing Videos with a Learned Autoregressive Generative Prior](https://arxiv.org/abs/2410.21264)", International Conference on Representation Learning, 2025.
